...
$$g(x) =\frac{1}{\sqrt{2\pi}} e^{-\frac{(x-2)^2}{2}}$$.
```{r}
h <- function(x) {x^2}
...
Since $2x-5 \leq (x-2)^2$, we can write $\exp(-\frac{(x-2)^2}{2}) \leq \exp(-\frac{2x-5}{2})$ and $g(x) \leq x^4\exp(-x)$. This inequality suggests we can use rejection sampling method with $Gamma(5,1)$ to generate a sample depend on $g(x)$.


```{r}
# density function of g(x)
dg <- function(x) x^4 * exp(-((x-2)^2)/2) / 25*sqrt(2 * pi)
# Rejection sampling method to generate random variable with g(x) as the pdf
rej_sam <- function(shape, scale){
  while(TRUE){
    x <- rgamma(1, shape = shape, scale = scale)
    u <- runif(1)
    if (u <= exp(-(x-2)^2)/exp(-2*x+5)) {
      return(x)
      break
      }
    }
  }
rg <- function(n){replicate(n, rej_sam(shape = 5, scale = 1))}
...
St <- function(s0=1, r=0.05, t, sigma){
  z <- rnorm(n = length(t), mean = 0, sd = 1)
  s0 * exp((0.05 - sigma^2/2)*t + sigma*sqrt(t)*z)
}
```
...
```{r}
PA <- function(n = 12, t, sigma, K){
  t_set <- c(1:n)*t/n
  SA <- mean(St(t = t_set,sigma = sigma))
  SAK_p <- ifelse(SA-K > 0, SA-K,0)
  exp(-r*t) * SAK_p
}

PA_rep <- function(nrep, t, sigma, K){
  replicate(nrep, PA(t=t,sigma = sigma,K = K))
}

PE <- function(t, sigma, K){
  St <-  St(t = t, sigma = sigma)
  St_p <- ifelse( St - K >0, St - K, 0)
  exp(-r*t) * St_p
}
...
PG <- function(n=12, t, sigma, K){
  t_set <- c(1:n)*t/n
  SG <- exp(mean(log(St(t = t_set,sigma=sigma))))
  SG_p <- ifelse(SG-K>0, SG-K,0)
  exp(-r*t) * SG_p
}

PG_rep <- function(nrep, t, sigma, K){
  replicate(nrep, PG(t=t, sigma=sigma, K = K))
}
